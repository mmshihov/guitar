...
\begin{document}
    ...
    \input{ch-intro}    %что-зачем
    %\input{ch-music}    %что есть музыка
    %\input{ch-notes}    %о нотной грамоте пару ласковых
    %\input{ch-guitar}   %об устройтсве гитары
    \input{ch-harmony}  %об ладах, интервалах и аккордах
    %\input{ch-tricks}   %нестандартные способы звукоизвлечения 
    \input{ch-concl}    %заключение
